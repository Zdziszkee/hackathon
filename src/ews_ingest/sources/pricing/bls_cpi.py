"""BLS CPI (spec §10): motor fuel & transportation services (free key)."""
...
@register_source("pricing.bls_cpi", scope=Scope.SECTOR_AGGREGATE)
class BlsCpi:
    """Pull CPI for motor fuel & transportation services."""

    source_id = "pricing.bls_cpi"
    source_type = SourceType.API

    def fetch(self, ctx: FetchContext) -> Iterator[RawRecord]:
        start_year = str(ctx.since.year) if ctx.since is not None else None
        for label, series_id in bls.CPI_SERIES.items():
            raw = bls.series_data(
                ctx.http,
                ctx.rate_policy,
                series_id=series_id,
